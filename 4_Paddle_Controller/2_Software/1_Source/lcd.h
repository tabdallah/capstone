...
// Function prototypes
void lcd_configure(void);			// Configure the LCD module
void lcd_data(unsigned char cx);		// Write an 8-bit value to the LCD
void lcd_cmd(unsigned char cx);		// Write an 8-bit command to the LCD
void lcd_puts(unsigned char *str);	// Writes a string to the LCD
void lcd_printf(char* format, ...);	// Printf for the LCD module